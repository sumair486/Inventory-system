            ...
            <div class="container-fluid">
                ...
                <div class="row">
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header">
                                <h4 class="card-title">  <a class="btn btn-success" href="{{ route('transaction.create') }}"> Create  Loader</a></h4>
                            </div><!--end card-header-->
                            <div class="card-body">
                                <div class="table-responsive">
                                    <table class="table" id="datatable_1">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                <td>{{ $key+1 }}</td>
                                                <td>{{ $transactions->commissions->name ?? '' }}</td>
                                                <td>{{ $transactions->border->address ?? ''  }}</td>
                                                <td>{{ $transactions->category->category_name ?? ''  }}</td>
                                                <td>{{ $transactions->product->product_name ?? '' }}</td>
                                                <td>{{ $transactions->total_quantity }}</td>
                                                <td>{{ date('d-m-Y', strtotime($transactions->date)) }}</td>
                                                {{-- <td>{{ $transactions->date }}</td> --}}

                                                {{-- <td>{{ $transactions->commissions->balance ?? '' }}</td> --}}



                                                <td>
                                                    <form action="{{ route('transaction.delete',$transactions->id) }}" method="POST">
                                                        <a class="btn btn-info" href="{{ route('transaction.edit',$transactions->id) }}"><i class="fa fa-edit"></i></a>
