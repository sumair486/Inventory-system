...
<head>
  <meta charset="utf-8"/>
  <title>Delivery Ledger List</title>
            ...
            <div class="container-fluid">
                ...
                <div class="row">
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header">
                                {{-- <h4 class="card-title">  <a class="btn btn-success" href="{{ route('commission_ledger.create') }}"> Create Commission Ledger</a></h4> --}}
                            </div><!--end card-header-->
                            <div class="card-body">
                                <div class="table-responsive">
                                    <table class="table" id="datatable_1">
                                        ...
                                        <tbody>
                                            @foreach ($commission_ledger as $key=>$commission_ledgers)
                                            <tr>
                                                <td>{{ $key+1 }}</td>
                                                <td>{{ $commission_ledgers->commissions->name ?? '' }}</td>
                                                <td>{{ $commission_ledgers->product->product_name ?? '' }}</td>
                                                <td>{{ $commission_ledgers->category->category_name ?? '' }}</td>
                                                <td>{{ $commission_ledgers->border->address ?? '' }}</td>
                                                <td>{{ $commission_ledgers->delivered_quantity }}</td>
                                                {{-- <td>{{ $commission_ledgers->remaining_quantity }}</td> --}}
                                                <td>{{ $commission_ledgers->transection_date}}</td>
                                                <td>{{ $commission_ledgers->balance}}</td>

                                              


                                                <td>
                                                    <form action="{{ route('commission_ledger.delete',$commission_ledgers->id) }}" method="POST">
                                                        {{-- <a class="btn btn-info" href="{{ route('commission_ledger.add',$commission_ledgers->id) }}">Add</a> --}}
                                                        {{-- @can('plan-edit')
